...
// create server READ ecg FIFO
int init_server()
{
    int server_fd;
    // create fifo first
    if (mkfifo(ECG_SERVER_NAME, PERMISSIONS) == -1){
    ...
    server_fd = open(ECG_SERVER_NAME, SERVER_READ_FLAGS); // file descriptor
    if (server_fd == -1){
        perror("Error: Unable to open server channel.");
        exit(EXIT_FAILURE);
    }

    printf("[ECG Server]: Server FIFO initialized with name: %s...\n", ECG_SERVER_NAME);
    return server_fd;
}
...
int retrieve_data(int *server_fd, struct data_content *data_retrieved, char* client_name)
{
    ...
    if (bytes_retrieved > 0){
        ...
        sprintf(client_name, ECG_CLIENT_NAME, data_retrieved->client_pid); // this name will be used to identify from the server
        printf("[ECG Server]: Bytes retrieved from %s: %d\n", client_name, data_retrieved->bytes_transfered);
        printf("[ECG Server]: Data retrieved from %s: %s\n", client_name, data_retrieved->data);
...
int connect_to_client(char* client_name){
    ...
    printf("[ECG Server]: Connection to %s established\n", client_name);
    return client_fd;
}
...
void transfer_data(char* client_name, struct data_content* transfer_data){
    ...
    if(bytes_transfered > 0){
        printf("[ECG Server]: Transfering data to %s, bytes transfered: %d\n", client_name, bytes_transfered);
        printf("[ECG Server]: Data successfully transfered.\n");
    }
}
...
int main(){
    ...
    printf("-------------------------------\n\n");
    printf("[SYSTEM]: ECG Server online...\n\n");

    int server_fd;
    char client_name[FILE_NAME_SIZE];
